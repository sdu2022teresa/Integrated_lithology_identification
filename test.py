...
def evaluate(val_loader,model,criterion,epoch):
    ...
    with torch.no_grad():
        for i,(input,target) in enumerate(val_loader):
            ...
            # output, _ = model(input)#inception V3
...
def main():
    ...
    # Build model
    model = torchvision.models.inception.inception_v3(pretrained=True)
    model.fc = nn.Linear(2048, config.num_classes)
    # model.fc = nn.Linear(1000, config.num_classes)#densenet
    model.cuda()
    ...
    for epoch in range(start_epoch,config.epochs):
        ...
        save_checkpoint({
                    "epoch":epoch + 1,
                    "model_name":config.model_name + '_' + str(epoch+1)+'_'+ str(best_precision1),
        ...
        torch.save(model.state_dict(), config.best_models + config.model_name + '/' + str(epoch+1) + '_' + str(valid_loss[1]) + '.pth')
